...
export async function executeTypeScriptCode(userCode, testCases, adapter, challengeId = DEFAULT_CHALLENGE) {
  if (!adapter) {
    throw new Error('Adapter is required for executeTypeScriptCode');
  }
...
class TestClock {
  private static _mockTime: number | null = null;
  ...
  static setCurrentTime(value: number) {
    TestClock._mockTime = value;
  }
  static reset() {
    TestClock._mockTime = null;
