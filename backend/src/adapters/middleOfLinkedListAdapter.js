/**
 * Adapter for Middle of the Linked List challenge
 * Handles middleNode(ListNode head) method that returns ListNode
 *
...
export default {
  ...
  extractInput: (testCase) => {
    // Middle of the Linked List test cases use 'head' field (array representation)
    return {
  ...
  generateInvocation: (parserVar) => {
    return `                    ListNode head = getTestHead(i);
                    actual = ${parserVar}.middleNode(head);`;
  },
...
${buildListNodeCode}
  ...
  getDefaultClassName: () => 'MiddleOfLinkedList',
